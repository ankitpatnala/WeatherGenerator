...
#
# The prescribed SST warming offset is taken from the RUN_ID suffix (..._0K/_2K/_4K) unless
# given explicitly as the 4th argument. It used to be hardcoded to 4, which silently applied
# +4K to every run regardless of its name -- do not put it back.
#
# Usage (submit the first link):
#   sbatch weathergen_decadal_chain.sh <RUN_ID> <NUM_STEPS> [MAX_LINKS] [OFFSET_K]
...
MAX=${3:-2}
# offset in K: explicit 4th arg, else parsed from the run name's trailing _<N>K
if [ -n "${4:-}" ]; then
  OFFSET=$4
elif [[ "$RUN_ID" =~ _([0-9]+(\.[0-9]+)?)K$ ]]; then
  OFFSET=${BASH_REMATCH[1]}
else
  echo "[chain] cannot infer SST offset from run_id '$RUN_ID'; pass it as the 4th argument" >&2
  exit 1
fi
# continuing a rollout MUST reuse the checkpoint that produced the saved latent
CKPT=wvlz4d8r
...
echo "[chain] run_id=$RUN_ID done=$DONE / $NUM_STEPS  links_left=$MAX  offset=${OFFSET}K  job=$SLURM_JOB_ID"
...
sbatch --dependency=afterany:"$SLURM_JOB_ID" "$SCRIPT" "$RUN_ID" "$NUM_STEPS" $((MAX - 1)) "$OFFSET" \
  || echo "[chain] WARNING: failed to submit successor job"

export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
srun uv --offline run inference --from-run-id "$CKPT" --run-id "$RUN_ID" \
  --options test_config.start_date=201001010000 test_config.end_date=202312310000 \
  ...
  streams.ERA5.max_num_targets=-1 streams.SST.warmup_offset_k.sst="$OFFSET"
