...
class LossLatentCosineMatching(LossModuleBase):
    """
    Band constraint on per-token cosine similarity between consecutive FE latent steps.
    ...
    def compute_loss(self, preds, targets, metadata, **kwargs) -> LossValues:
        ...
        losses_all: dict = {"cosine_band": 0.0}
        ...
        for step_pred in preds.latent:
            ...
            if cos_sim is None:
                continue
            # Two-sided hinge: penalise tokens outside [cosine_low, cosine_high]
            step_loss = (
        ...
        losses_all["cosine_band"] = loss.detach().item()
