...
class MultiStreamDataSampler(torch.utils.data.IterableDataset):
    def __init__(
        ...
        self.data_path = cf.data_paths[0]
    ...
    def _init_condition_stream(self, stream_info: dict) -> None:
        ...
        filename = stream_info.get("filename", None)

        folder_path = pathlib.Path(self.data_path)

        logger.info(f"Initializing condition stream with file {filename} in folder {folder_path}")

        ds = DataReaderCondition(
            tw_handler=self.time_window_handler, 
            stream_info=stream_info, 
            filename=folder_path / filename if filename is not None else None
        )
    ...
    def get_condition_num_channels(self):
        return ( sum([ds[0].num_channels for _, ds in self.condition_datasets.items()]),
                 sum([len(ds[0].source_channels) for _, ds in self.condition_datasets.items()])
                )
